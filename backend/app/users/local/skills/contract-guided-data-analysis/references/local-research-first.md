...
| Step | Path |
|------|------|
| Domain | `/workspace/source/contracts/{domain}/domain-knowledge.md` |
| Metrics | `/workspace/source/contracts/{domain}/metric-index.md` |
| Golden eval | `/workspace/source/contracts/{domain}/eval/golden_cases.md` (if file exists) |
| Special logic | `/workspace/source/ref/{domain}/special_logic.txt`, `table list.txt`, `table relationship.txt` (if present for domain) — always check `special_logic.txt` for the resolved table(s), see [`special-logic-check.md`](special-logic-check.md) |
| Storage metadata | `/workspace/target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/` (table/column metadata), `/workspace/target/storage/wkb/indexes/**` |
| Tables | `/workspace/target/knowledgebase/{domain}/{stem}.md` (L1/L2/L3/L6 as needed); **NEVER** read `/workspace/source/contracts/{domain}/tables/*.md` |

WKB catalogue is local — use `python /workspace/target/storage/wkb/indexing/run_query.py`, not full-tree grep.
...
| Tool / path | Rule |
|-------------|------|
| `/workspace/source/contracts/**/golden-questions.md` | **Never** read, cite, or match |
| `/workspace/source/contracts/b-report-us/tables/**`, `/workspace/source/contracts/pos/tables/**` | **NEVER** read for any request in this skill, at any stage |
...
- [ ] Special logic checked (`/workspace/source/ref/{domain}/special_logic.txt`, if present for domain)
