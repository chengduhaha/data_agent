# DIM: Shared dimension for B Report attribute enrichment and join lookups (`dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view`)
...
- evidence_source: source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md
- knowledgebase_path: target/knowledgebase/b-report-us/dim_pub_sales_hierarchy_primary_role_by_terr_view.md
- contract_source: source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md
...
|--------|---------|---------------|-------|
| Hive | yes | `dim_${country}.dim_pub_sales_hierarchy_primary_role_by_terr_view` | Documented in B Report contract v2 |
| Vertica | yes | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | Contract marks Vertica verified |
...
| **l1_catalog_seed** | `target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/vertica_dim_us_dim_pub_sales_hierarchy_primary_role_by_terr_view.json` |
| **column_count** | 22 |
| **partition_keys** | `none` |
| **ddl_source** | B Report contract catalog — `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md` |
| **retrieval** | `python -m tools.wkb.indexing.run_query --query "b-report-us dim_pub_sales_hierarchy_primary_role_by_terr_view schema" --intent find_table_schema` |

### Lineage
- **upstream:** Curated DIM/ODS load jobs (per contract) — `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md:L1`
- **downstream:** B Report fact/DM serving tables and dashboards — `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md:L1`
...
| Expected completion | 02:30-04:00 PT. |
...
- Primary lookup keys: `sub_group_id`, `group_id`, `sales_rep_id`, `sup_id`, `mgr_id`, `dir_id`, `vp_id`
...
```sql
-- See contract L3 Standard Time-Filter SQL in source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md
```

### End-to-end flow
1. Upstream DIM/ODS load populates `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` (ETL not in b-report-us Bitbucket snapshot).
2. B Report fact and serving jobs join this dimension for attribute enrichment.
3. Vertica consumers query `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` for reporting.

```mermaid
flowchart LR
  upstream["Upstream DIM/ODS loads"]
  dim["dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view"]
...
|--------|------------------|
| `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | Primary dimension catalog object (contract v2) |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | overwrite / incremental | `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md:L5` | yes |
| **Hive alternative** | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | same as reporting table | — | B Report contract | — |
...
|--------|-------|----------|
| B Report contract | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md` |

#### Downstream consumers (verified)
| Object / script | Evidence |
|-----------------|----------|
| B Report hub and serving tables | `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md:L6` — see contract L6 |

#### Operational detail (verified)
- Load pattern: Not documented in repository — `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md:L1`
...
- `dim_pub_sales_hierarchy_primary_role_by_terr_view.md` — B Report contract source — `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md`
- `target/knowledgebase/pos/dim_pub_sales_hierarchy_primary_role_by_terr_view.md` — cross-domain Knowledgebase entry for same table object

---

*Document generated from `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md` (contract_v2).*
