# DIM: Shared dimension for B Report attribute enrichment and join lookups (`dim_us.dim_pub_sales_cust_type`)
...
- evidence_source: source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md
- knowledgebase_path: target/knowledgebase/b-report-us/dim_pub_sales_cust_type.md
- contract_source: source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md
...
- **Natural key:** `entry_id`, `gl_dept_no`, `manager_id`, `backup_id`, `key`, `level`
- **Exclusions:** Non-US schemas, backup/temp table variants (`_bkp`, `_temp`), and non-shipped-order scenarios.

### Cross-engine presence
| Engine | Present | Canonical FQN | Notes |
|--------|---------|---------------|-------|
| Hive | yes | `dim_${country}.dim_pub_sales_cust_type` | Documented in B Report contract v2 |
| Vertica | yes | `dim_us.dim_pub_sales_cust_type` | Contract marks Vertica verified |
...
| **l1_catalog_seed** | `target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/vertica_dim_us_dim_pub_sales_cust_type.json` |
| **column_count** | 15 |
| **partition_keys** | `none` |
| **ddl_source** | B Report contract catalog — `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md` |
| **retrieval** | `python -m tools.wkb.indexing.run_query --query "b-report-us dim_pub_sales_cust_type schema" --intent find_table_schema` |

### Lineage
- **upstream:** Curated DIM/ODS load jobs (per contract) — `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md:L1`
- **downstream:** B Report fact/DM serving tables and dashboards — `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md:L1`
...
- Primary lookup keys: `entry_id`, `gl_dept_no`, `manager_id`, `backup_id`
...
```sql
-- See contract L3 Standard Time-Filter SQL in source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md
```

### End-to-end flow
1. Upstream DIM/ODS load populates `dim_us.dim_pub_sales_cust_type` (ETL not in b-report-us Bitbucket snapshot).
2. B Report fact and serving jobs join this dimension for attribute enrichment.
3. Vertica consumers query `dim_us.dim_pub_sales_cust_type` for reporting.

```mermaid
flowchart LR
  upstream["Upstream DIM/ODS loads"]
  dim["dim_us.dim_pub_sales_cust_type"]
...
|--------|------------------|
| `dim_us.dim_pub_sales_cust_type` | Primary dimension catalog object (contract v2) |
...
SELECT entry_id, gl_dept_no, manager_id, date_flag, COUNT(*) AS cnt
FROM dim_us.dim_pub_sales_cust_type
WHERE date_flag = '${partition_value}'
GROUP BY entry_id, gl_dept_no, manager_id, date_flag
HAVING COUNT(*) > 1;
```
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_sales_cust_type` | `dim_us.dim_pub_sales_cust_type` | overwrite / incremental | `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md:L5` | yes |
| **Hive alternative** | `dim_us.dim_pub_sales_cust_type` | same as reporting table | — | B Report contract | — |
...
|--------|-------|----------|
| B Report contract | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md` |

#### Downstream consumers (verified)
| Object / script | Evidence |
|-----------------|----------|
| B Report hub and serving tables | `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md:L6` — see contract L6 |

#### Operational detail (verified)
- Load pattern: Not documented in repository — `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md:L1`
...
- `dim_pub_sales_cust_type.md` — B Report contract source — `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md`
- `target/knowledgebase/customer/dim_pub_sales_cust_type.md` — cross-domain Knowledgebase entry for same table object

---

*Document generated from `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md` (contract_v2).*
