...
def exec_main_sql(country, date_flag, dt_month, etl_timestamp):
    ...
    insert overwrite table dw_${country}.dws_disty_brpt_vpl_mtd partition(date_flag = '${date_flag}')
    select
    ${month_no},
    coalesce(table_dws.vpl_no,table_goal.vpl_no,-3)               as vpl_no,
    table_vpl.vpl_code                                            as vpl_code,
    coalesce(table_dws.vpc_group_id,table_vpl.vpc_group_id,-3)    as vpc_group_id, --特殊逻辑和pl_extend_comb一致
    table_vpl.vpc_group_desc                                      as vpc_group_desc,
    coalesce(table_dws.vend_no,table_goal.vend_no,-3)             as vend_no,
    table_vend.vend_name                                          as vend_name,
    coalesce(table_dws.master_vend_no,table_vend.master_vend_no,-3)       as master_vend_no,
    table_mvend.cis_mk_name                                       as master_vend_name,
    nvl(table_dws.seg_code,table_goal.seg_code)                   as seg_code,
    ...
    '${etl_timestamp}',
    coalesce(table_dws.pm_id,table_vpl_hierarchy.pm_id,table_vend_hierarchy.pm_id,-3)                           as pm_id,
    coalesce(table_dws.pm_mgr_id,table_vpl_hierarchy.pm_manager_id,table_vend_hierarchy.pm_mgr_id,-3)           as pm_mgr_id,
    coalesce(table_dws.pm_dir_id,table_vpl_hierarchy.pm_director_id,table_vend_hierarchy.pm_dir_id,-3)          as pm_dir_id,
    coalesce(table_dws.pm_vp_id,table_vpl_hierarchy.pm_vp_id,table_vend_hierarchy.pm_vp_id,-3)                  as pm_vp_id,
    coalesce(table_dws.buyer_id, table_vpl_hierarchy.buyer_id,table_vpl_hierarchy.pana_id,-3)                       as buyer_id,
    coalesce(table_dws.buyer_mgr_id, table_vpl_hierarchy.buyer_manager_id,table_vpl_hierarchy.pana_manager_id,-3)   as buyer_mgr_id,
    coalesce(table_dws.buyer_dir_id, table_vpl_hierarchy.buyer_director_id,table_vpl_hierarchy.pana_director_id,-3) as buyer_dir_id,
    coalesce(table_dws.buyer_vp_id, table_vpl_hierarchy.buyer_vp_id,table_vpl_hierarchy.pana_vp_id,-3)              as buyer_vp_id,
    ...
    on table_dws.vpl_no = table_goal.vpl_no
    and table_dws.vend_no = table_goal.vend_no
    ...
    on nvl(table_dws.vpl_no,table_goal.vpl_no) = table_vpl.vpl_no
    ...
    on nvl(table_dws.vpl_no,table_goal.vpl_no) = table_vpl_hierarchy.vpl_no
    ...
    on nvl(table_dws.vend_no,table_goal.vend_no) = table_vend_hierarchy.vend_no
    """)
