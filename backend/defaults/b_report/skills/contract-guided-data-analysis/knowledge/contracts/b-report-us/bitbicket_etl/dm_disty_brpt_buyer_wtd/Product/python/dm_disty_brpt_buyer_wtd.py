...
def main():
    ...
    run_sql("""
    insert overwrite table dm_${country}.dm_disty_brpt_buyer_wtd partition(dt_week)
    select
        ...
        sum(reg_inv),
        sum(reg_inv_age0_30),
        sum(reg_inv_age31_60),
        sum(reg_inv_age61_90),
        sum(reg_inv_age90_up),
        sum(rma_inv),
        sum(rma_inv_age0_30),
        sum(rma_inv_age31_60),
        sum(rma_inv_age61_90),
        sum(rma_inv_age90_up),
        sum(oh_cost),
        sum(oo_cost),
        sum(oh_qty),
        sum(oo_qty),
        ...
        table_dim.week_flag
    from (select * from dm_${country}.dm_disty_brpt_buyer_1d
