...
select
    ...
    sum( nvl(rr_unit, 0) ) as rr_unit,
    sum( nvl(rr_sales, 0) ) as rr_sales,
    sum( nvl(rr_cost, 0) ) as rr_cost,
    sum( nvl(rr_gm, 0) ) as rr_gm,
    sum( nvl(rr_ngm, 0) ) as rr_ngm,
    sum( nvl(rr_opl, 0) ) as rr_opl,
    sum( nvl(rr_cgp, 0) ) as rr_cgp,
    sum( nvl(rr_total_btl, 0) ) as rr_total_btl,
    sum( nvl(rr_tgm, 0) ) as rr_tgm,
    ...
    sum( nvl(rr_oplgm_plus_amt,0) ) as rr_oplgm_plus_amt
...
-- select * from dim_${country}.dim_pub_customer_info_df
--       where date_flag = '${first_biz_date}'
--       and data_source = 'CIS'
--       and delete_datetime IS NULL
--       and is_restricted <> 'Y'
--       and is_discontinued <> 'Y'
...
insert overwrite dw_${country}.dws_disty_brpt_bd_mtd partition(date_flag = '${date_flag}')
select
    table_mi_tmp.month_no,
...
insert into dw_${country}.dws_disty_brpt_bd_mtd partition(date_flag = '${date_flag}')
select
    table_mi_tmp.month_no,
...
from (select *
      ...
      where date_flag = '${date_flag}') as table_left
...
select
*,
row_number() over(partition by project_no,task_no,
                               sku_no,vpl_no,vend_no,
                               cust_no,cust_terr,cust_type
                  order by month_no) as rank
from table_tmp)

insert overwrite table dw_${country}.dws_disty_brpt_bd_mtd partition(date_flag = '${date_flag}')
select
month_no,
project_no,
project_name,
