# -*- coding: utf-8 -*-
# @Time : 10/6/2023 11:06 AM
# @Author : Marvin Ma

from synnex.bigdata import conf
from synnex.bigdata.pyspark import run_sql

# dw_{country}.dws_disty_brpt_cross_cvv_mtd
# dw_{country}.dws_disty_brpt_cross_cvv_1d
...
def exec_main_sql(country, date_flag, dt_month, etl_timestamp):
    ...
    from dw_{country}.dws_disty_brpt_cross_cvv_1d
    ...
    insert overwrite table dw_{country}.dws_disty_brpt_cross_cvv_comb_mtd partition(date_flag = '{date_flag}')
    select
        {month_no},
    ...
    from (select *
          from dw_${country}.dws_disty_brpt_cross_cvv_comb_mtd
    ...
    on coalesce(table_dwd.pm_id,table_vpl_hierarchy.pm_id,table_vend_hierarchy.pm_id) = table_manager.userid
    left join ods_${country}.ods_cis_corp_manager as table_manager2   --unique id : userid
    on coalesce(table_dwd.pm_mgr_id,table_vpl_hierarchy.pm_manager_id,table_vend_hierarchy.pm_mgr_id) = table_manager2.userid
    left join ods_${country}.ods_cis_corp_manager as table_manager3   --unique id : userid
    on coalesce(table_dwd.pm_dir_id,table_vpl_hierarchy.pm_director_id,table_vend_hierarchy.pm_dir_id) = table_manager3.userid
    left join ods_${country}.ods_cis_corp_manager as table_manager4   --unique id : userid
    on coalesce(table_dwd.pm_vp_id,table_vpl_hierarchy.pm_vp_id,table_vend_hierarchy.pm_vp_id) = table_manager4.userid
    """)
