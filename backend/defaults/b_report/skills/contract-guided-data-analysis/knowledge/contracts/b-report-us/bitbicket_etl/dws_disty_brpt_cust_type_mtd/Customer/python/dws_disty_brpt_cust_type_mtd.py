...
"""
# dw_{country}.dws_disty_brpt_terr_mtd
...
def exec_main_sql(country, date_flag, dt_month, etl_timestamp):
    ...
    insert overwrite table dw_${country}.dws_disty_brpt_cust_type_mtd partition(date_flag = '${date_flag}')
    select 
        ${month_no},
        coalesce(table_dws.cust_type,table_goal.cust_type,-3) as cust_type,
        table_type.cust_type_descr                            as cust_type_desc,
    ...
    on table_dws.cust_type = table_goal.cust_type
    and table_dws.division = table_goal.division
    and (table_dws.company_no = table_goal.company_no or table_goal.company_no = -1)
    
    left join ods_${country}.ods_cis_corp_cust_type as table_type
    on nvl(table_dws.cust_type,table_goal.cust_type) = table_type.cust_type
