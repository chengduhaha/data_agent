...
**Domain:** b-report-us  
**Source contract:** `source/contracts/b-report-us/tables/dws_disty_brpt_cust_type_comb_mtd.md`  
**Knowledgebase path:** `target/knowledgebase/b-report-us/dws_disty_brpt_cust_type_comb_mtd.md`
...
|-------|------------------|
| **Catalog object** | `dw_us.dws_disty_brpt_cust_type_comb_mtd` — DWS layer table used in US B Report analytics (`US B Report baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dw_us.dws_disty_brpt_cust_type_comb_mtd` | `dw_us.dws_disty_brpt_cust_type_comb_mtd` | overwrite / incremental | B Report contract `dws_disty_brpt_cust_type_comb_mtd.md:L1` | yes (B Report contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dw_us.dws_disty_brpt_cust_type_comb_mtd` | same as reporting table | - | B Report contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dw_us.dws_disty_brpt_cust_type_comb_mtd`** in Vertica for B Report reporting aligned to this contract.
...
- **Natural key:** `company_no`, `date_flag`
...
SELECT company_no, date_flag, order_line_no, month_no, COUNT(*) AS cnt
FROM dw_us.dws_disty_brpt_cust_type_comb_mtd
WHERE month_no = '${partition_value}'
GROUP BY company_no, date_flag, order_line_no, month_no
HAVING COUNT(*) > 1;
```
...
- `m_gm` — m gm
- `m_ngm` — m ngm
- ... and 116 additional measure columns (see column register)
...
**Target table:** `dw_us.dws_disty_brpt_cust_type_comb_mtd`  
...
flowchart LR
  ...
  tgt["dw_us.dws_disty_brpt_cust_type_comb_mtd"]
...
|--------|-----------------|
| `dw_us.dws_disty_brpt_cust_type_comb_mtd` | Primary catalog table documented from B Report contract |
...
|--------|-------|----------|
| B Report contract source | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dws_disty_brpt_cust_type_comb_mtd.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| B Report consumers | `dws_disty_brpt_cust_type_comb_mtd.md:L6` — see contract L6 |

### Operational detail (verified)

- Freshness: Not documented in repository
- Column count: 149 (B Report contract catalog)
...
*Document generated from B Report contract `source/contracts/b-report-us/tables/dws_disty_brpt_cust_type_comb_mtd.md`.*
