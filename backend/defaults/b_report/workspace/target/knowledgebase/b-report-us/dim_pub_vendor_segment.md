...
**Domain:** b-report-us  
**Source contract:** `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md`  
**Knowledgebase path:** `target/knowledgebase/b-report-us/dim_pub_vendor_segment.md`
...
|-------|------------------|
| **Catalog object** | `dim_us.dim_pub_vendor_segment` — DIM layer table used in US B Report analytics (`US B Report baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_vendor_segment` | `dim_us.dim_pub_vendor_segment` | overwrite / incremental | B Report contract `dim_pub_vendor_segment.md:L1` | yes (B Report contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dim_us.dim_pub_vendor_segment` | same as reporting table | - | B Report contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dim_us.dim_pub_vendor_segment`** in Vertica for B Report reporting aligned to this contract.
...
- **Natural key:** `vend_no`, `master_vend_no`
...
SELECT vend_no, master_vend_no, order_line_no, date_flag, COUNT(*) AS cnt
FROM dim_us.dim_pub_vendor_segment
WHERE date_flag = '${partition_value}'
GROUP BY vend_no, master_vend_no, order_line_no, date_flag
HAVING COUNT(*) > 1;
```
...
**Target table:** `dim_us.dim_pub_vendor_segment`  
...
flowchart LR
  ...
  tgt["dim_us.dim_pub_vendor_segment"]
...
|--------|-----------------|
| `dim_us.dim_pub_vendor_segment` | Primary catalog table documented from B Report contract |
...
|--------|-------|----------|
| B Report contract source | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| B Report consumers | `dim_pub_vendor_segment.md:L6` — see contract L6 |
...
*Document generated from B Report contract `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md`.*
