...
**Domain:** b-report-us  
**Source contract:** `source/contracts/b-report-us/tables/dws_disty_brpt_cust_type_wtd.md`  
**Knowledgebase path:** `target/knowledgebase/b-report-us/dws_disty_brpt_cust_type_wtd.md`
...
|-------|------------------|
| **Catalog object** | `dw_us.dws_disty_brpt_cust_type_wtd` — DWS layer table used in US B Report analytics (`US B Report baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dw_us.dws_disty_brpt_cust_type_wtd` | `dw_us.dws_disty_brpt_cust_type_wtd` | overwrite / incremental | B Report contract `dws_disty_brpt_cust_type_wtd.md:L1` | yes (B Report contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dw_us.dws_disty_brpt_cust_type_wtd` | same as reporting table | - | B Report contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dw_us.dws_disty_brpt_cust_type_wtd`** in Vertica for B Report reporting aligned to this contract.
...
- **Natural key:** `week_no`, `company_no`
...
SELECT week_no, company_no, order_line_no, date_flag, COUNT(*) AS cnt
FROM dw_us.dws_disty_brpt_cust_type_wtd
WHERE date_flag = '${partition_value}'
GROUP BY week_no, company_no, order_line_no, date_flag
HAVING COUNT(*) > 1;
```
...
**Target table:** `dw_us.dws_disty_brpt_cust_type_wtd`  
...
flowchart LR
  ...
  tgt["dw_us.dws_disty_brpt_cust_type_wtd"]
...
|--------|-----------------|
| `dw_us.dws_disty_brpt_cust_type_wtd` | Primary catalog table documented from B Report contract |
...
|--------|-------|----------|
| B Report contract source | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dws_disty_brpt_cust_type_wtd.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| B Report consumers | `dws_disty_brpt_cust_type_wtd.md:L6` — see contract L6 |

### Operational detail (verified)

- Freshness: Not documented in repository
- Column count: 108 (B Report contract catalog)
...
*Document generated from B Report contract `source/contracts/b-report-us/tables/dws_disty_brpt_cust_type_wtd.md`.*
