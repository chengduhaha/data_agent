...
**Domain:** b-report-us  
**Source contract:** `source/contracts/b-report-us/tables/dim_pub_sales_division.md`  
**Knowledgebase path:** `target/knowledgebase/b-report-us/dim_pub_sales_division.md`
...
|-------|------------------|
| **Catalog object** | `dim_us.dim_pub_sales_division` — DIM layer table used in US B Report analytics (`US B Report baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_sales_division` | `dim_us.dim_pub_sales_division` | overwrite / incremental | B Report contract `dim_pub_sales_division.md:L1` | yes (B Report contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dim_us.dim_pub_sales_division` | same as reporting table | - | B Report contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dim_us.dim_pub_sales_division`** in Vertica for B Report reporting aligned to this contract.
...
- **Natural key:** `manager_id`, `backup_id`, `entry_id`
...
- `division_sdesc` — division sdesc
- `division_sort` — division sort
...
**Target table:** `dim_us.dim_pub_sales_division`  
...
flowchart LR
  ...
  tgt["dim_us.dim_pub_sales_division"]
...
|--------|-----------------|
| `dim_us.dim_pub_sales_division` | Primary catalog table documented from B Report contract |
...
|--------|-------|----------|
| B Report contract source | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_pub_sales_division.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| B Report consumers | `dim_pub_sales_division.md:L6` — see contract L6 |
...
*Document generated from B Report contract `source/contracts/b-report-us/tables/dim_pub_sales_division.md`.*
