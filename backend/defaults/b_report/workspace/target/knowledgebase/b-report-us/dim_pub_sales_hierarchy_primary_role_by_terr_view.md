...
**Domain:** b-report-us  
**Source contract:** `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md`  
**Knowledgebase path:** `target/knowledgebase/b-report-us/dim_pub_sales_hierarchy_primary_role_by_terr_view.md`
...
|-------|------------------|
| **Catalog object** | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` — DIM layer table used in US B Report analytics (`US B Report baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | overwrite / incremental | B Report contract `dim_pub_sales_hierarchy_primary_role_by_terr_view.md:L1` | yes (B Report contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | same as reporting table | - | B Report contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view`** in Vertica for B Report reporting aligned to this contract.
...
|--------|-----------------|
| `dim_us.dim_pub_sales_hierarchy_primary_role_by_terr_view` | Primary catalog table documented from B Report contract |
...
|--------|-------|----------|
| B Report contract source | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| B Report consumers | `dim_pub_sales_hierarchy_primary_role_by_terr_view.md:L6` — see contract L6 |
...
*Document generated from B Report contract `source/contracts/b-report-us/tables/dim_pub_sales_hierarchy_primary_role_by_terr_view.md`.*
