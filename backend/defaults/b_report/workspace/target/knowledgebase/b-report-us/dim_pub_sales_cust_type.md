...
**Domain:** b-report-us  
**Source contract:** `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md`  
**Knowledgebase path:** `target/knowledgebase/b-report-us/dim_pub_sales_cust_type.md`
...
|-------|------------------|
| **Catalog object** | `dim_us.dim_pub_sales_cust_type` — DIM layer table used in US B Report analytics (`US B Report baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_sales_cust_type` | `dim_us.dim_pub_sales_cust_type` | overwrite / incremental | B Report contract `dim_pub_sales_cust_type.md:L1` | yes (B Report contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dim_us.dim_pub_sales_cust_type` | same as reporting table | - | B Report contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dim_us.dim_pub_sales_cust_type`** in Vertica for B Report reporting aligned to this contract.
...
- **Natural key:** `entry_id`, `gl_dept_no`, `manager_id`, `backup_id`
...
- `credit_risk_rate` — credit risk rate
- `bo_expire_days` — bo expire days
...
**Target table:** `dim_us.dim_pub_sales_cust_type`  
...
flowchart LR
  ...
  tgt["dim_us.dim_pub_sales_cust_type"]
...
|--------|-----------------|
| `dim_us.dim_pub_sales_cust_type` | Primary catalog table documented from B Report contract |
...
|--------|-------|----------|
| B Report contract source | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| B Report consumers | `dim_pub_sales_cust_type.md:L6` — see contract L6 |
...
*Document generated from B Report contract `source/contracts/b-report-us/tables/dim_pub_sales_cust_type.md`.*
