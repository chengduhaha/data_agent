...
|-------|------------------|
| **Catalog object** | `dlake_dg.dwd_disty_inv_rio_req_detail` — DLAKE layer table used in US POS reporting (`US POS baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dlake_dg.dwd_disty_inv_rio_req_detail` | `dlake_dg.dwd_disty_inv_rio_req_detail` | overwrite / incremental | POS contract `dwd_disty_inv_rio_req_detail.md:L1` | yes (POS contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dlake_dg.dwd_disty_inv_rio_req_detail` | same as reporting table | - | POS contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dlake_dg.dwd_disty_inv_rio_req_detail`** in Vertica for POS-domain reporting aligned to this contract.
...
- **Natural key:** `rio_req_no`, `inproc_ref_no`, `entry_id`, `region_no`
...
FROM dlake_dg.dwd_disty_inv_rio_req_detail
...
SELECT rio_req_no, inproc_ref_no, entry_id, date_flag, COUNT(*) AS cnt
FROM dlake_dg.dwd_disty_inv_rio_req_detail
WHERE date_flag = '${partition_value}'
GROUP BY rio_req_no, inproc_ref_no, entry_id, date_flag
HAVING COUNT(*) > 1;
```
...
- `rio_req_line` — rio req line
- `inproc_ref_type` — inproc ref type
- `inproc_ref_no` — inproc ref no
- `entry_id` — entry id
- `entry_datetime` — entry datetime
- `inproc_ref_line` — inproc ref line
- `fixed` — fixed
- `region_no` — region no
...
**Target table:** `dlake_dg.dwd_disty_inv_rio_req_detail`  
...
flowchart LR
  ...
  tgt["dlake_dg.dwd_disty_inv_rio_req_detail"]
...
|--------|-----------------|
| `dlake_dg.dwd_disty_inv_rio_req_detail` | Primary catalog table documented from POS contract |
...
| Object | Usage | Evidence |
|--------|-------|----------|
| POS contract source | Table metadata, grain, columns | `C:\Users\T154858D.TDSNX\Desktop\git_repo_v1\data_analysis_agent_brpt\knowledge\POS\tables\dwd_disty_inv_rio_req_detail.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| POS RDS / export consumers | `dwd_disty_inv_rio_req_detail.md:L6` — Vertica RDS POS report scripts (`rds_*_rtv`), ad-hoc POS exports |
...
*Document generated from POS contract `C:\Users\T154858D.TDSNX\Desktop\git_repo_v1\data_analysis_agent_brpt\knowledge\POS\tables\dwd_disty_inv_rio_req_detail.md`.*