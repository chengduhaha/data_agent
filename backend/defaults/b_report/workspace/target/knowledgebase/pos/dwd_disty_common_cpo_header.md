# Catalog object: `dw_us.dwd_disty_common_cpo_header`

- artifact_type: etl_table
- artifact_id: dw_us.dwd_disty_common_cpo_header
- domain: pos
...
- **Table:** `dw_us.dwd_disty_common_cpo_header`
...
| Field | Value |
|-------|-------|
| **entity_id** | `dw_us.dwd_disty_common_cpo_header` |
| **l1_catalog_seed** | `target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/{engine}_{schema}_{table}.json` |
| **column_count** | pending |
| **partition_keys** | See preserved Grain |
| **ddl_source** | pending |
| **retrieval** | `python -m tools.wkb.indexing.run_query --query "pos dwd_disty_common_cpo_header schema" --intent find_table_schema` |

### Lineage
- **Load ETL in wiki repo:** Not documented in repository (`source/contracts/pos/bitbucket-etl/dwd_disty_common_cpo_header/` absent)
...
- Prefer Vertica `dw_us.dwd_disty_common_cpo_header` for reporting when contract documents Vertica sync.
...
```mermaid
flowchart LR
  CONTRACT["POS contract catalog"] --> T["dw_us.dwd_disty_common_cpo_header"]
...
### Step-by-step logic
1. No bitbucket-etl bundle for `dwd_disty_common_cpo_header` in this repository.
2. Consumers that reference `dwd_disty_common_cpo_header` are listed under L6 when found by repo scan.
...
- No `source/contracts/pos/bitbucket-etl/dwd_disty_common_cpo_header/` bundle in this repository.
