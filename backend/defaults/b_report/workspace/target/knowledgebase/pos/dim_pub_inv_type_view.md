# Catalog object: `dim_us.dim_pub_inv_type_view`

- artifact_type: etl_table
- artifact_id: dim_us.dim_pub_inv_type_view
- domain: pos
...
- **Table:** `dim_us.dim_pub_inv_type_view`
...
| Field | Value |
|-------|-------|
| **entity_id** | `dim_us.dim_pub_inv_type_view` |
| **l1_catalog_seed** | `target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/{engine}_{schema}_{table}.json` |
| **column_count** | pending |
| **partition_keys** | See preserved Grain |
| **ddl_source** | pending |
| **retrieval** | `python -m tools.wkb.indexing.run_query --query "pos dim_pub_inv_type_view schema" --intent find_table_schema` |

### Lineage
- **Load ETL in wiki repo:** Not documented in repository (`source/contracts/pos/bitbucket-etl/dim_pub_inv_type_view/` absent)
...
- Prefer Vertica `dim_us.dim_pub_inv_type_view` for reporting when contract documents Vertica sync.
...
```mermaid
flowchart LR
  CONTRACT["POS contract catalog"] --> T["dim_us.dim_pub_inv_type_view"]
...
### Step-by-step logic
1. No bitbucket-etl bundle for `dim_pub_inv_type_view` in this repository.
2. Consumers that reference `dim_pub_inv_type_view` are listed under L6 when found by repo scan.
...
- No `source/contracts/pos/bitbucket-etl/dim_pub_inv_type_view/` bundle in this repository.
...
**Domain:** pos  
**Source contract:** `C:\Users\T154858D.TDSNX\Desktop\git_repo_v1\data_analysis_agent_brpt\knowledge\POS\tables\dim_pub_inv_type_view.md`  
**Knowledgebase path:** `target/knowledgebase/pos/dim_pub_inv_type_view.md`
...
|-------|------------------|
| **Catalog object** | `dim_us.dim_pub_inv_type_view` — DIM layer table used in US POS reporting (`US POS baseline`). |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_inv_type_view` | `dim_us.dim_pub_inv_type_view` | overwrite / incremental | POS contract `dim_pub_inv_type_view.md:L1` | yes (POS contract v2 — Vertica verified in source catalog) |
| **Hive alternative** | `dim_us.dim_pub_inv_type_view` | same as reporting table | - | POS contract cross-engine note | - |
| **ETL internal** | n/a | n/a | - | ETL not in wiki repo | - |

Business users should query **`dim_us.dim_pub_inv_type_view`** in Vertica for POS-domain reporting aligned to this contract.
...
- **Natural key:** `entry_id`, `gl_acct_no`
...
FROM dim_us.dim_pub_inv_type_view
...
SELECT entry_id, gl_acct_no, order_line_no, date_flag, COUNT(*) AS cnt
FROM dim_us.dim_pub_inv_type_view
WHERE date_flag = '${partition_value}'
GROUP BY entry_id, gl_acct_no, order_line_no, date_flag
HAVING COUNT(*) > 1;
```
...
**Target table:** `dim_us.dim_pub_inv_type_view`  
...
flowchart LR
  ...
  tgt["dim_us.dim_pub_inv_type_view"]
...
|--------|-----------------|
| `dim_us.dim_pub_inv_type_view` | Primary catalog table documented from POS contract |
...
| Object | Usage | Evidence |
|--------|-------|----------|
| POS contract source | Table metadata, grain, columns | `C:\Users\T154858D.TDSNX\Desktop\git_repo_v1\data_analysis_agent_brpt\knowledge\POS\tables\dim_pub_inv_type_view.md` |

### Downstream consumers (verified)

| Object / script | Evidence |
|-----------------|----------|
| POS RDS / export consumers | `dim_pub_inv_type_view.md:L6` — Vertica RDS POS report scripts (`rds_*_rtv`), ad-hoc POS exports |
...
*Document generated from POS contract `C:\Users\T154858D.TDSNX\Desktop\git_repo_v1\data_analysis_agent_brpt\knowledge\POS\tables\dim_pub_inv_type_view.md`.*