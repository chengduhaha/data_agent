...
- evidence_source: source/contracts/b-report-us/tables/dim_disty_bd_project_user.md
- knowledgebase_path: target/knowledgebase/b-report-us/dim_disty_bd_project_user.md
- contract_source: source/contracts/b-report-us/tables/dim_disty_bd_project_user.md
...
|--------|---------|---------------|-------|
| Hive | yes | `dim_${country}.dim_disty_bd_project_user` | Documented in B Report contract v2 |
| Vertica | yes | `dim_us.dim_disty_bd_project_user` | Contract marks Vertica verified |

### Physical schema reference
| Field | Value |
|-------|-------|
| **Authoritative catalog** | WKB L1 storage seed (not duplicated in this file) |
| **entity_id** | `dim_us.dim_disty_bd_project_user` |
| **l1_catalog_seed** | `target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/vertica_dim_us_dim_disty_bd_project_user.json` |
| **column_count** | 15 |
| **partition_keys** | `none` |
| **ddl_source** | B Report contract catalog — `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md` |
| **retrieval** | `python -m tools.wkb.indexing.run_query --query "b-report-us dim_disty_bd_project_user schema" --intent find_table_schema` |

### Lineage
- **upstream:** Curated DIM/ODS load jobs (per contract) — `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md:L1`
- **downstream:** B Report fact/DM serving tables and dashboards — `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md:L1`
...
| Expected completion | 02:00-03:00 PT. |
...
- Primary lookup keys: `project_no`, `task_no`, `bd_rep_id`, `bd_mgr_id`, `bd_dir_id`, `bd_vp_id`, `bd_svp_id`
...
```sql
-- See contract L3 Standard Time-Filter SQL in source/contracts/b-report-us/tables/dim_disty_bd_project_user.md
```

### End-to-end flow
1. Upstream DIM/ODS load populates `dim_us.dim_disty_bd_project_user` (ETL not in b-report-us Bitbucket snapshot).
2. B Report fact and serving jobs join this dimension for attribute enrichment.
3. Vertica consumers query `dim_us.dim_disty_bd_project_user` for reporting.

```mermaid
flowchart LR
  upstream["Upstream DIM/ODS loads"]
  dim["dim_us.dim_disty_bd_project_user"]
...
|--------|------------------|
| `dim_us.dim_disty_bd_project_user` | Primary dimension catalog object (contract v2) |
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_disty_bd_project_user` | `dim_us.dim_disty_bd_project_user` | overwrite / incremental | `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md:L5` | yes |
| **Hive alternative** | `dim_us.dim_disty_bd_project_user` | same as reporting table | — | B Report contract | — |
...
|--------|-------|----------|
| B Report contract | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md` |

#### Downstream consumers (verified)
| Object / script | Evidence |
|-----------------|----------|
| B Report hub and serving tables | `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md:L6` — see contract L6 |

#### Operational detail (verified)
- Load pattern: Not documented in repository — `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md:L1`
...
- `dim_disty_bd_project_user.md` — B Report contract source — `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md`
- No additional cross-domain Knowledgebase entries with matching table stem.

---

*Document generated from `source/contracts/b-report-us/tables/dim_disty_bd_project_user.md` (contract_v2).*
