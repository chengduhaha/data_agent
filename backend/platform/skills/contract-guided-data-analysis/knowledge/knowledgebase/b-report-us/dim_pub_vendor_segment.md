# DIM: Shared dimension for B Report attribute enrichment and join lookups (`dim_us.dim_pub_vendor_segment`)
...
- evidence_source: source/contracts/b-report-us/tables/dim_pub_vendor_segment.md
- knowledgebase_path: target/knowledgebase/b-report-us/dim_pub_vendor_segment.md
- contract_source: source/contracts/b-report-us/tables/dim_pub_vendor_segment.md
...
- **Natural key:** `vend_no`, `master_vend_no`, `key`, `level`, `one`, `row`
- **Exclusions:** Non-US schemas, backup/temp table variants (`_bkp`, `_temp`), and non-shipped-order scenarios.

### Cross-engine presence
| Engine | Present | Canonical FQN | Notes |
|--------|---------|---------------|-------|
| Hive | yes | `dim_${country}.dim_pub_vendor_segment` | Documented in B Report contract v2 |
| Vertica | yes | `dim_us.dim_pub_vendor_segment` | Contract marks Vertica verified |

### Physical schema reference
| Field | Value |
|-------|-------|
| **Authoritative catalog** | WKB L1 storage seed (not duplicated in this file) |
| **entity_id** | `dim_us.dim_pub_vendor_segment` |
| **l1_catalog_seed** | `target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/vertica_dim_us_dim_pub_vendor_segment.json` |
| **column_count** | 17 |
| **partition_keys** | `none` |
| **ddl_source** | B Report contract catalog — `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md` |
| **retrieval** | `python -m tools.wkb.indexing.run_query --query "b-report-us dim_pub_vendor_segment schema" --intent find_table_schema` |

### Lineage
- **upstream:** Curated DIM/ODS load jobs (per contract) — `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md:L1`
- **downstream:** B Report fact/DM serving tables and dashboards — `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md:L1`
...
- Primary lookup keys: `vend_no`, `master_vend_no`
...
```sql
-- See contract L3 Standard Time-Filter SQL in source/contracts/b-report-us/tables/dim_pub_vendor_segment.md
```

### End-to-end flow
1. Upstream DIM/ODS load populates `dim_us.dim_pub_vendor_segment` (ETL not in b-report-us Bitbucket snapshot).
2. B Report fact and serving jobs join this dimension for attribute enrichment.
3. Vertica consumers query `dim_us.dim_pub_vendor_segment` for reporting.

```mermaid
flowchart LR
  upstream["Upstream DIM/ODS loads"]
  dim["dim_us.dim_pub_vendor_segment"]
...
|--------|------------------|
| `dim_us.dim_pub_vendor_segment` | Primary dimension catalog object (contract v2) |
...
SELECT vend_no, master_vend_no, key, date_flag, COUNT(*) AS cnt
FROM dim_us.dim_pub_vendor_segment
WHERE date_flag = '${partition_value}'
GROUP BY vend_no, master_vend_no, key, date_flag
HAVING COUNT(*) > 1;
```
...
|------|-------------|----------------|-----------|----------|--------------|
| **Query for reporting** | `dim_us.dim_pub_vendor_segment` | `dim_us.dim_pub_vendor_segment` | overwrite / incremental | `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md:L5` | yes |
| **Hive alternative** | `dim_us.dim_pub_vendor_segment` | same as reporting table | — | B Report contract | — |
...
|--------|-------|----------|
| B Report contract | Table metadata, grain, columns | `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md` |

#### Downstream consumers (verified)
| Object / script | Evidence |
|-----------------|----------|
| B Report hub and serving tables | `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md:L6` — see contract L6 |

#### Operational detail (verified)
- Load pattern: Not documented in repository — `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md:L1`
- Column count: 17 (B Report contract catalog)
...
- `dim_pub_vendor_segment.md` — B Report contract source — `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md`
- No additional cross-domain Knowledgebase entries with matching table stem.

---

*Document generated from `source/contracts/b-report-us/tables/dim_pub_vendor_segment.md` (contract_v2).*
