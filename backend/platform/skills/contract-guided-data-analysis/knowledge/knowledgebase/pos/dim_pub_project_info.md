# Catalog object: `dim_us.dim_pub_project_info`

- artifact_type: etl_table
- artifact_id: dim_us.dim_pub_project_info
- domain: pos
...
- **Table:** `dim_us.dim_pub_project_info`
...
| Field | Value |
|-------|-------|
| **entity_id** | `dim_us.dim_pub_project_info` |
| **l1_catalog_seed** | `target/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/{engine}_{schema}_{table}.json` |
| **column_count** | pending |
| **partition_keys** | See preserved Grain |
| **ddl_source** | pending |
| **retrieval** | `python -m tools.wkb.indexing.run_query --query "pos dim_pub_project_info schema" --intent find_table_schema` |

### Lineage
- **Load ETL in wiki repo:** Not documented in repository (`source/contracts/pos/bitbucket-etl/dim_pub_project_info/` absent)
...
- Prefer Vertica `dim_us.dim_pub_project_info` for reporting when contract documents Vertica sync.
...
```mermaid
flowchart LR
  CONTRACT["POS contract catalog"] --> T["dim_us.dim_pub_project_info"]
...
### Step-by-step logic
1. No bitbucket-etl bundle for `dim_pub_project_info` in this repository.
2. Consumers that reference `dim_pub_project_info` are listed under L6 when found by repo scan.
...
- No `source/contracts/pos/bitbucket-etl/dim_pub_project_info/` bundle in this repository.
