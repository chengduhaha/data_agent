...
def exec_sql():
    ...
    from (select
          *
          ...
          where date_flag between '${firstday_of_month}' and '${date_flag}') as table_inv
    ...
    insert overwrite table dw_${country}.dws_disty_brpt_part_1d partition (date_flag)
    select
        ...
        coalesce(table_dwd.date_flag,table_tmp_inv.date_flag,table_tmp_inv2.date_flag) as date_flag
    from table_dwd

    left join table_tmp_inv
    on table_dwd.sku_no = table_tmp_inv.sku_no
    and table_dwd.date_flag = table_tmp_inv.date_flag
    and table_dwd.company_no = table_tmp_inv.company_no

    left join table_tmp_inv2
    on table_dwd.sku_no = table_tmp_inv2.sku_no
    and table_dwd.date_flag = table_tmp_inv2.date_flag
    and table_dwd.company_no = table_tmp_inv2.company_no
    """)
    ...
    left join (select *
                from (select
                          ...
                          master_vend_no,
                          master_vend_name,
                          row_number() over(partition by date_flag,vend_no order by master_vend_no) as rank --假如dave的表中存在一对多的情况，随便选一个
                      from dw_${country}.dws_disty_brpt_pl_extend_1d
                      where date_flag between '${firstday_of_month}' and '${date_flag}'
                      group by
                          date_flag,
    ...
    and table_dwd.date_flag = table_old_vend.date_flag
    ...
    left join (select *
                from (select
                          ...
                          pm_vp_id,
                          seg_code,
                          row_number() over(partition by date_flag,vpl_no order by vpc_group_id) as rank --假如dave的表中存在一对多的情况，随便选一个
                      from dw_${country}.dws_disty_brpt_pl_extend_1d
                      where date_flag between '${firstday_of_month}' and '${date_flag}'
                      group by
                          date_flag,
    ...
    and table_dwd.date_flag = table_old_vpl.date_flag
    """)
    ...
    select 
        ...
        table_mvend.vend_name                                               as master_vend_name,
