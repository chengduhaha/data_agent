...
drop table if exists table_ca_scm_reference_8329;
create local temporary table table_ca_scm_reference_8329 on commit preserve rows as
with sales_hierarchy
as (
    select distinct
         s.sales_terr
    from dim_ca.dim_pub_sales_hierarchy_by_terr_user_role s
    inner join dim_ca.dim_pub_manager m
...
as (
    select distinct
         pm.vend_no
        ,pm.vpl_no
    from dim_ca.dim_pub_pm_vpc_matrix pm
    inner join dim_ca.dim_pub_manager m
...
as (
    select distinct
         b.cust_no
    from dim_ca.dim_pub_bd_hierarchy a
    inner join dim_ca.dim_disty_bd_project_cust b
...
as (
    ...
    from dim_ca.dim_pub_bd_hierarchy a
    inner join dim_ca.dim_disty_bd_project_sku b
            on a.project_no = b.project_no
    where a.loginid = 'sandra.macdonald'
      and a.project_no = -1
    )
...
as (
    select row_number() over (
        ...
        ,fact.prod_type
        ,fact.company_no
    from dw_ca.dwd_disty_common_pos_di fact
    left join dim_ca.dim_pub_part_info part
    ...
    where 1 = 1
      ...
      and fact.order_type not in (
            ...
            from dim_ca.dim_pub_order_type
            where ship_tran_no is null
            )
      and fact.vend_no in (54356,54424)
    )
    ,com_list
as (
    select
    ...
    from all_list al
    inner join dw_ca.dwd_disty_common_pos_di com
...
from final_data fd
inner join dw_ca.dwd_pub_common_shipped_order_scm_spa_detail_di he
        on fd.order_no = he.order_no
       and fd.order_type = he.order_type
       and fd.order_line_no = he.order_line_no
;

drop table if exists table_ca_pos_8329;
create local temporary table table_ca_pos_8329 on commit preserve rows as
with sales_hierarchy as (
    ...
    ,all_vpg as (
        ...
        from dim_ca.dim_pub_vpc_group_xref_view b
        inner join dim_ca.dim_pub_vpc_group_view a
    ...
    ,all_list as (
        ...
        from dw_ca.dwd_disty_common_pos_di fact
        inner join dim_ca.dim_pub_part_info part
                on fact.sku_no = part.sku_no
        inner join dim_ca.dim_pub_customer_info cust
                on fact.bill_to_cust_no = cust.cust_no
        left join dim_ca.dim_pub_sku_profile_extend sku_profile
    ...
    ,contract_list as (
        ...
        from dw_ca.dwd_stellr_billing_history_di
    ...
    ,recalculate_exp as (
        ...
        from all_list al
        inner join dw_ca.dwd_pub_common_shipped_order_scm_spa_detail_di scm
    ...
    ,final_data as (
        ...
        inner join final_order_list fol
               ...
               and al.order_no = fol.order_no
               and al.order_line_no = fol.order_line_no
        left join dm_ca.dm_disty_pos_order_kit_di rc
    ...
    ,ser_number_list as (
        ...
        inner join dw_ca.dwd_disty_common_order_serial_no_di ser
...
select
  ...
  b.spa_ref_no,
  b.pri_approv_ref_no
from table_ca_pos_8329 a
left join table_ca_scm_reference_8329 b
