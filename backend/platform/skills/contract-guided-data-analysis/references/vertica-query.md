...
**Prerequisite:** Local research gate passed. SQL compiled from contracts / `metric-index` (`golden_cases` disabled by default — see [`golden-cases-match.md`](golden-cases-match.md)). Vertica is never the first source consulted.
...
Grain, columns, partition keys come from `knowledge/contracts/**` and WKB L1 snapshots.
