...
| ranking / comparison sense | Comparative words lack a baseline and the choice is material |

`breakdown_dimension` is a **query_spec / GROUP BY angle field**, not an ask-first clarification gate. For attribution / root-cause / why / what-drove / top-drivers **without** a named angle:

- set `needs_clarification=false` (unless another hard slot above is open, e.g. time);
- set `pipeline_mode=diagnose`;
- populate `query_spec.breakdown_candidates` and/or `driver_dimensions` from pack-supported dimensions;
- do **not** ask the user which dimension to try.

Do **not** set clarification for missing table, formula, flow, or ETL logic — those are KB gaps (fail closed). See [output-contract.md](./output-contract.md) Ambiguity Handling and [analysis-clarification.md](./analysis-clarification.md).

Set `needs_clarification=true` **only** when the question or data is genuinely ambiguous and blocks routing; if clear enough to proceed, keep `needs_clarification=false` and do not ask.
...
Top-N, YoY, trend, and scalar KPI questions are typically `lookup`. Use `diagnose` when the user asks why / what drove / root cause / top drivers — including when no breakdown angle is named (auto-explore pack-supported `breakdown_candidates` / `driver_dimensions`; do not ask first).
