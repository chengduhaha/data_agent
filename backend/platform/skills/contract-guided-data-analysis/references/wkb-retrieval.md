...
Use two-stage indexing under `knowledge/storage/wkb/` to short-list table/metric candidates before opening knowledgebase markdown. Run after [`special-logic-check.md`](special-logic-check.md).

---

## Storage locations

- Snapshots: `knowledge/storage/wkb/snapshots/_snapshot_id_template/`
- Sparse index: `knowledge/storage/wkb/indexes/sparse_prefilter/`
- Semantic layers: `knowledge/storage/wkb/indexes/semantic/l1_catalog|l2_usage|l3_code|l4_flow|l5_eval/`

---

## Storage layer (`l1_catalog`) metadata search

Before opening knowledgebase docs, search table/column metadata directly under `knowledge/storage/wkb/snapshots/_snapshot_id_template/l1_catalog/`:
...
3. Open `knowledge/knowledgebase/{domain}/{stem}.md` (same domain) — **NEVER** read `knowledge/contracts/{domain}/tables/*.md`
...
| L5 `l5_eval` | Validation ideas; `eval/golden_cases.md` disabled by default — only when user explicitly asks (see [`golden-cases-match.md`](golden-cases-match.md)) |
...
If `wkb_query.py` fails:

- Report index unavailable
- Resolve routing from `metric-index.md` + domain-knowledge only
- Do not broaden to a repository-wide ETL source grep or Bitbucket
...
Then open `knowledge/knowledgebase/b-report-us/dws_disty_brpt_vend_mtd.md` L3/L2 sections only.
