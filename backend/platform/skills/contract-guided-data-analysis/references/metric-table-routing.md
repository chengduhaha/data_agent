...
5. If multiple **user-facing** metric definitions conflict and the question does not disambiguate → ask only when that ambiguity blocks routing; otherwise prefer pack selection guidance. Do **not** ask the user for KB-owned formulas or table choice — fail closed if the pack cannot resolve.
...
4. If dedup logic cannot be determined from KB, fail closed / **no data found** — do **not** ask the user to invent dedup rules.
...
Do require clarification **only** when time is genuinely ambiguous or missing for the question shape:

- `temporal_obligation = user_must_specify`; or
- Stage-2 cannot find certified default-period assembly.

If time is already clear (user-specified, entity-anchored, or KB-default confirmed), do **not** ask.
